...
// Extending solely to expose the protected static getOptions() for the test.
class TestVAOptions extends VAOptions {
    public static getTestOptions(): any {
        return VAOptions.getOptions();
    }
}
...
test("getOptions", done => {
    ...
    TestUtil.InitializeI18NOnce().then(() => {
        ...
        const options: any = TestVAOptions.getTestOptions();

        expect(options).toBeDefined();
        expect(options).toEqual(JSON.parse(expectedOptions));
